  ...
  root.getStatus = function(wallet, opts, cb) {
    ...
    function cacheBalance(wallet, balance) {
      ...
      rateService.whenAvailable(function() {
        ...
        var totalBalanceAlternative = rateService.toFiat(cache.totalBalanceSat, cache.alternativeIsoCode, wallet.coin)* xmcc_to_btc;
        var pendingBalanceAlternative = rateService.toFiat(cache.pendingAmount, cache.alternativeIsoCode, wallet.coin)* xmcc_to_btc;
        var lockedBalanceAlternative = rateService.toFiat(cache.lockedBalanceSat, cache.alternativeIsoCode, wallet.coin)* xmcc_to_btc;
        var spendableBalanceAlternative = rateService.toFiat(cache.spendableAmount, cache.alternativeIsoCode, wallet.coin)* xmcc_to_btc;
        var alternativeConversionRate = rateService.toFiat(100000000, cache.alternativeIsoCode, wallet.coin)* xmcc_to_btc;
