...
angular.module('copayApp.controllers').controller('amountController', function ($scope, $filter, $timeout, $ionicScrollDelegate, $ionicHistory, gettextCatalog, platformInfo, lodash, configService, rateService, $stateParams, $window, $state, $log, txFormatService, ongoingProcess, popupService, bwcError, payproService, profileService, bitcore, amazonService, nodeWebkitService, $http) {
  ...
  function processAmount() {
    ...
    if (lodash.isNumber(result)) {
      ...
      if (availableUnits[unitIndex].isFiat) {
        ...
        if (a) {
          // $scope.alternativeAmount = (parseFloat(txFormatService.formatAmount(a * unitToSatoshi, true)) * xmcc_to_usd).toFixed(8);
          $scope.alternativeAmount = (a / xmcc_to_btc).toFixed(4);
        } else {
          if (result) {
        ...
        console.log('b');
        // $scope.alternativeAmount = (parseFloat($filter('formatFiatAmount')(toFiat(result))) * xmcc_to_usd).toFixed(8);
        $scope.alternativeAmount = (result * xmcc_to_usd).toFixed(4);
      }
    }
  };
  ...
  $scope.finish = function () {
    ...
    if ($scope.nextStep) {
      ...
      if (unit.isFiat) {
        amount = (fromFiat(amount) * unitToSatoshi / xmcc_to_btc).toFixed(0);
      } else {
