...
@Component({
  selector: 'page-colossusxt',
  templateUrl: 'colossusxt.html',
})
export class ColossusXTPage {
  private walletsBTC: any[];
  private walletsColossusXT: any[];
  private errors: any;
  ...
  ionViewWillEnter() {
    ...
    let xPubKeyIndex = lodash.keyBy(this.walletsColossusXT, "credentials.xPubKey");
    ...
    lodash.each(this.availableWallets, (wallet) => {
      this.walletProvider.getBalance(wallet, { coin: 'colx' }).then((balance) => {
        wallet.bchBalance = this.txFormatProvider.formatAmountStr('colx', balance.availableAmount);
        wallet.error = null;
      }).catch((err) => {
  ...
  public duplicate(wallet: any) {
    this.logger.debug('Duplicating wallet for COLOSSUSXT: ' + wallet.id + ': ' + wallet.name);

    let opts: any = {
      name: wallet.name + '[COLOSSUSXT]',
      m: wallet.m,
      n: wallet.n,
    ...
    function addCopayers(newWallet, isNew, cb) {
      ...
      this.logger.info('Adding copayers for COLOSSUSXT wallet config:' + wallet.m + '-' + wallet.n);
