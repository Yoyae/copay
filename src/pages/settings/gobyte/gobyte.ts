...
@Component({
  selector: 'page-gobyte',
  templateUrl: 'gobyte.html',
})
export class GoBytePage {
  private walletsBTC: any[];
  private walletsGoByte: any[];
  private errors: any;
  ...
  ionViewWillEnter() {
    ...
    let xPubKeyIndex = lodash.keyBy(this.walletsGoByte, "credentials.xPubKey");
    ...
    lodash.each(this.availableWallets, (wallet) => {
      this.walletProvider.getBalance(wallet, { coin: 'gbx' }).then((balance) => {
        wallet.bchBalance = this.txFormatProvider.formatAmountStr('gbx', balance.availableAmount);
        wallet.error = null;
      }).catch((err) => {
  ...
  public duplicate(wallet: any) {
    this.logger.debug('Duplicating wallet for GOBYTE: ' + wallet.id + ': ' + wallet.name);

    let opts: any = {
      name: wallet.name + '[GOBYTE]',
      m: wallet.m,
      n: wallet.n,
    ...
    function addCopayers(newWallet, isNew, cb) {
      ...
      this.logger.info('Adding copayers for GOBYTE wallet config:' + wallet.m + '-' + wallet.n);
